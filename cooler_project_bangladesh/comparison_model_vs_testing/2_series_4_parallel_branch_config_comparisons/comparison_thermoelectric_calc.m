...
if thermo_chip == "TEC1-12706"
    ...
    % Main Data
    current_input_arr = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C14:I14');
    cooling_power_test_arr = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C49:I49');
    power_input_test_arr = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C60:I60');
    COP_test_arr = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C64:I64');
    delta_temp_test_arr = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C43:I43');
    average_inlet_air_temp = 273.15 + xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'L13');

    % Error bars
    J_input_err = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C17:I17');
    cooling_power_test_err = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C50:I50');
    power_input_test_err = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C61:I61');
    COP_test_err = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C65:I65');
    delta_temp_test_err = xlsread("Experimental Results.xlsx", "Current (TEC 1-12710)-Fan 12V", 'C44:I44');
end
...
CFM_nominal_cold = 29.24;   % Back-calculated from emprical testing (See spreadsheet)                                
...
CFM_nominal_hot = 77.36 * num_fans;   % Back-calculated from emprical testing (See spreadsheet)                                
CFM_fan_hot = CFM_nominal_hot * 1;       % Per Parallel Branch [CubicFt/min]
volumetric_flow_rate_hot = CFM_fan_hot * ((0.3048^3) / 60);  % m^3/s - conversion factor
m_dot_air_hot = volumetric_flow_rate_hot * rho_air;
fan_area_hot = (pi * 0.07^2 - pi * 0.025^2) * num_fans;                 % CHANGEME
...
for i = 1:length(delta_J_arr)
    ...
    for j = 1:num_modules_series
        ...
        Q_c_peltier = double(sol.z) * power_factor;        % Already factored in cold efficiency and all channels...
        ...
        power_required_total = power_required_total + (power_required * num_parallel_branches * power_factor);
